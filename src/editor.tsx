...
import { composeDnaInstructions, loadDna } from './dna'
import { makeSnapshot, putSnapshot } from './spaces'
import { importSkillFile, skillToMarkdown } from './skillfile'
...
export function Editor({ requestedProjectId, onExit, onProjectsChanged }: { requestedProjectId?: string; onExit: () => void; onProjectsChanged?: () => void }) {
  ...
  /** AI instructions = enabled skills + the user's creative DNA. */
  const aiInstructions = () => [composeSkillInstructions(skills), composeDnaInstructions(loadDna())].filter(Boolean).join('\n\n')
  ...
  useEffect(() => {
    const bootWorkspace = async () => {
      try {
        ...
        const first = localProjects.find((project) => project.id === requestedProjectId) ?? localProjects[0]
  ...
  const saveSnapshotToArchive = async () => {
    const label = window.prompt('Name this snapshot (e.g. "Brand direction")', `${new Date().toLocaleDateString(undefined, { month: 'short', day: 'numeric' })} snapshot`)?.trim()
    if (!label) return
    try {
      await putSnapshot(makeSnapshot(activeProjectId, fileName, label, cloneNodes(nodesRef.current)))
      setShowExport(false)
      setToast('Snapshot saved to your Archive')
    } catch {
      setToast('Could not save the snapshot')
    }
  }
  ...
  const runWireframe = async () => {
    ...
    try {
      setPendingWireframe(await generateWireframe(aiSettings, prompt, aiInstructions()))
    } catch (error) {
      handleAiError(error)
    } finally {
      setAiBusy('')
  ...
  const runReview = async () => {
    ...
    setReviewResult('')
    try {
      setReviewResult(await reviewDesign(aiSettings, buildDesignSummary(nodesRef.current), aiInstructions()))
    } catch (error) {
      handleAiError(error)
    } finally {
      setAiBusy('')
  ...
  const runCopySuggestions = async () => {
    ...
    setCopyOptions(null)
    try {
      setCopyOptions(await suggestCopy(aiSettings, selected.text ?? '', selected.name, aiInstructions()))
    } catch (error) {
      handleAiError(error)
    } finally {
      setAiBusy('')
    ...
    <main className="app-shell">
      <header className="topbar">
        <div className="file-control">
          <button className="brand-mark" aria-label="Back to your workspace" title="Back to your workspace" onClick={onExit}><span /><span /><span /></button>
          <button className="editor-home" title="Back to your workspace" aria-label="Back to your workspace" onClick={onExit}><ChevronRight size={14} style={{ transform: 'rotate(180deg)' }} /></button>
        ...
        <div className="top-actions">
          ...
          <div className="export-wrap">
            ...
            {showExport && <div className="popover export-popover">
              ...
              <button onClick={() => { void saveSnapshotToArchive() }}><Star size={16} /><span><b>Snapshot to Archive</b><small>Named version of this design</small></span><ArrowUpRight size={15} /></button>
...
function SkillsModal({ skills, onChange, onClose }: { skills: AiSkill[]; onChange: (skills: AiSkill[]) => void; onClose: () => void }) {
  ...
  const importFromFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return
    try {
      const parsed = await importSkillFile(file)
      const skill: AiSkill = {
        id: `skill-${Date.now()}`,
        name: parsed.name,
      ...
      setImportNote({ ok: true, message: `Imported “${parsed.name}”${parsed.resources.length ? ` with ${parsed.resources.length} reference file${parsed.resources.length === 1 ? '' : 's'}` : ''} — it's on and will guide every AI request.` })
    } catch (error) {
      setImportNote({ ok: false, message: error instanceof Error ? error.message : 'Could not read this skill file.' })
    } finally {
      event.target.value = ''
    }
  }
  ...
  const exportSkill = (skill: AiSkill) => {
    const link = document.createElement('a')
    link.href = URL.createObjectURL(new Blob([skillToMarkdown(skill)], { type: 'text/markdown' }))
    link.download = `${skill.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'skill'}.SKILL.md`
    link.click()
    window.setTimeout(() => URL.revokeObjectURL(link.href), 800)
  }
  const resetForm = () => {
  ...
  return (
    <div className="modal-scrim" onClick={onClose}>
      <div className="modal skills-modal" role="dialog" aria-label="AI skills" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="modal-body">
          <p className="skills-intro">Skills are extra instructions added to every AI request, so answers match how <b>you</b> work. Turn on what you like, write your own, or <b>import a skill file</b> — a <code>SKILL.md</code>, README, or a zip bundle in the same format Claude Skills use. They are saved on this device permanently.</p>
          <div className="skill-import">
            <button className="tour-secondary" onClick={() => skillFileRef.current?.click()}><Download size={14} /> Import skill (.md / .zip)</button>
            <small>Frontmatter <code>name:</code> / <code>description:</code> is read when present; extra files in a zip become reference material for the AI.</small>
            <input ref={skillFileRef} className="visually-hidden" type="file" accept=".md,.markdown,.txt,.zip,text/markdown,text/plain,application/zip" onChange={(event) => { void importFromFile(event) }} />
          </div>
          {importNote && <p className={`skill-import-note ${importNote.ok ? 'is-ok' : 'is-fail'}`}>{importNote.message}</p>}
          <div className="skills-list">
            {skills.map((skill) => <div className={`skill-row ${skill.enabled ? 'is-on' : ''}`} key={skill.id}>
              <div className="skill-text">
                <b>{skill.name}{skill.source === 'file' && <span className="skill-badge" title="Imported from a skill file">FILE</span>}</b>
                <small>{skill.builtin ? skill.description : skill.source === 'file' ? `${skill.description}${skill.resources?.length ? ` · ${skill.resources.length} reference file${skill.resources.length === 1 ? '' : 's'}` : ''}` : skill.instructions.slice(0, 110) + (skill.instructions.length > 110 ? '…' : '')}</small>
              </div>
              <div className="skill-actions">
                {!skill.builtin && <button className="skill-edit" title="Export as SKILL.md" onClick={() => exportSkill(skill)}><Share2 size={13} /></button>}
